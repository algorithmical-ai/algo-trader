...
async def run_weekly_put_wheel(session):
    ...
    tickers = ["NVDA", "AAPL", "MSFT", "AMD", "TSLA", "META", "GOOGL", "AMZN", "SMCI", "AVGO"]
    ...
    for ticker in tickers:
        try:
            ...
            if best_put is not None:
                reason = (f"Wheel Put | Δ{best_put['delta']} | {best_put['distance_pct']}% OTM | "
                          f"IVR {iv_rank_val:.0f} | Credit ${best_put['premium']:.2f}")
                await send_signal(
                    ticker, "sell_to_open_put", reason,
    ...
    for pos in alpaca_positions:
        ...
        if qty > 0 and ticker in WheelTracker.get_open_puts():
            ...
            if best_call is not None:
                ...
                await send_signal(
                    ticker, "sell_to_open_call", reason,
