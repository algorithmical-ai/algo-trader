...
from app.src.data.unusual_whales import get_flow_signal, get_congress_trades, get_dark_pool, get_iv_rank, get_screener_tickers
...
async def evaluate_ticker(ticker: str, df_1m, df_daily, session):
    try:
        ...
        df_daily_t = df_daily.xs(ticker, level=0) if ticker in df_daily.index.get_level_values(0) else None
        ...
        if not pos and high_iv:
            if current_time <= orb_end_time:
                if price > orb_high and price > vwap_val and is_uptrend(df_daily_t) and flow == "bullish" and congress == "bullish" and dark == "bullish":
                    ...
                    return
                if price < orb_low and price < vwap_val and is_downtrend(df_daily_t) and flow == "bearish":
                ...
                if price < vwap_val and is_uptrend(df_daily_t) and flow == "bullish" and congress == "bullish":
        ...
        if pos:
            ...
            pnl_pct = ((price - entry_price) / entry_price) * 100 if "buy_to_open" in entry_action else ((entry_price - price) / entry_price) * 100

            exit_flow = await get_flow_signal(ticker, session)
            if (pnl_pct >= 2.0 and "buy_to_open" in entry_action and exit_flow == "bearish") or (pnl_pct <= -2.0 and "sell_to_open" in entry_action and exit_flow == "bullish") or current_time >= time.fromisoformat(settings.TRADING_END):
                reason = f"Target Hit + Flow Exit | PnL: {pnl_pct:+.2f}%"
                exit_action = "sell_to_close" if "buy_to_open" in entry_action else "buy_to_close"
